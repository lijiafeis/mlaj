...
class WeixinController extends Controller{
	...
	function __construct(){
		...
		if(S('access_token')){
			...
			S('access_token',$this->access_token,7200);
		}
	}
	...
	function get_qr($scene_id){
		...
		if($ress['errcode'] == '45009'){
			return '今日创建二维码已超出1w个，超过上限，请明天再试。';exit;
		}
		if(!is_dir('Public/erweima/qrimg/')){
			mkdir('Public/erweima/qrimg/');
		}
		$file_name = 'Public/erweima/qrimg/'.$scene_id.'.jpg';
		file_put_contents('Public/erweima/qrimg/'.$scene_id.'.jpg',$ress);
		return $file_name;
	}
	...
	function get_prepay_id($openid,$total_fee,$out_trade_no,$notify_url,$pay_id,$good_name){
		...
		$result = $this->http_request($url,$data);//dump($result);exit;
	...
	function save_head_pic($user_id){
		...
		if(!is_dir('Public/erweima/head_pic/')){
			mkdir('Public/erweima/head_pic/');
		}
		file_put_contents('Public/erweima/head_pic/'.$user_id.'.jpg',$res);
	...
	//客服接口发送视频消息
	function send_vedio($openid,$media_id){
		$url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=".$this->access_token;
		$data='{
			"touser":"'.$openid.'",
			"msgtype":"video",
