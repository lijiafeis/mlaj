...
class IndexController extends Controller{
    ...
    public function valid()
    {
        ...
        $nonce = $_GET["nonce"];
        $tmpArr = array('weixin', $timestamp, $nonce);
        sort($tmpArr);
   ...
   //响应消息
    ...
    private function receiveEvent($object)
    {
        ...
        switch ($object->Event)
        {
            case "subscribe": //关注事件
				...
				if(empty($object->EventKey) && $system['subscribe_all'] != 1){
					$users = M('users');
					$res = $users  -> where(" openid = '$object->FromUserName' ") ->find();				
					if($res['user_id']){
						$id = $res['user_id'];
					...
					$res = $this -> save_user_path($object,str_replace("qrscene_","",$object->EventKey));
					$content .= $res."\n".$subscribe['content'] ;
				}
                break;
            case "unsubscribe"://取消关注事件
                ...
                break;
            case "SCAN": //扫描事件
            	$content = "请在取消关注状态扫描二维码，完成信息注册与关系锁定 ";
                break;
            case "CLICK": //菜单点击事件
    ...
    private function save_user_path($object,$scene_id){
    	...
    	$path_info = $users -> field("openid,nickname") -> where(" user_id = '$scene_id' ") -> find();
    	...
    	if(!$res['user_id']){   		 			  		
    		...
    		return "死党,我是「".$path_info['nickname']."」 感谢你对教练技术的信任和支持!";
			//return "感谢您由「".$path_info['nickname']."」推荐成为第".$return.'位关注者';
    	}else{
    		$return =  $res['user_id'];
    	...
    	$weixin -> send_word($path_info['openid'],"系统通知：\n\n您的好友「".$data['nickname']."」已成功关注，感谢你对教练技术的支持与认可。");   	  	
    }
