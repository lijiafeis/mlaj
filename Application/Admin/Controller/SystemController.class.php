...
class SystemController extends ActionController{
	function __construct(){
        ...
        $auth=new \Think\Auth();
        $res = $auth->check(CONTROLLER_NAME,session('admin_id'));
        if(!$res){
            $this->error('您没有权限管理该版块，请联系管理员',U('index/main'));
        }
	}
	/* 权限组列表视图 */
	function admin(){
	...
	public function image(){
		$image_info = M('sy_image');
		if($_POST['uplode'] == 1){
			foreach($_POST as $val){
				if($val != 1 && $val != ''){
					$image_info -> add(array('pic_url'=>$val,'time'=>time()));
				}
			}
			$this -> success("幻灯片图片上传成功");
			exit;
		}
		$info  = $image_info -> order("time desc") -> select();
		$this ->assign("empty",'<div class="text-center">暂无数据</div>');
		$this ->assign("image",$info);
		$this -> display();
	}
